...
export function useOpenAISpeechRecognition({
      ...
      try {
        ...
        dataChannel.current.onmessage = (event) => {
          ...
          try {
            ...
            if (window.__openAIRawTranscription) {
              // For listen mode, we need to be more lenient - don't wait for isComplete flag
              // This ensures smoother playback, especially for Arabic to English translation
              const isListenPage = window.location.pathname.includes('/listen');
              const shouldProcess = isListenPage 
                ? (window.__openAIRawTranscription.translatedText?.trim() || '')
                : window.__openAIRawTranscription.isComplete;
              ...
              if (shouldProcess) {
                ...
                if (sourceText && translatedText) {
                  ...
                  // For listen mode, use our global registry of processed messages with timestamps
                  if (isListenPage) {
                    // Create a message fingerprint - use only the first 20 chars to avoid overly specific matching
                    const messageKey = `${sourceText.substring(0, 20)}-${translatedText.substring(0, 20)}`;
                    
                    // Use a global cache with timestamps for listen mode messages
                    const processedMessagesMap = (window as any).__listenModeProcessedMessages = (window as any).__listenModeProcessedMessages || {};
                    const now = Date.now();
                    const lastProcessed = processedMessagesMap[messageKey] || 0;
                    
                    // Only log messages that were processed very recently (within 1 second)
                    // But ALWAYS continue processing in listen mode for smoother experience
                    if (lastProcessed && (now - lastProcessed < 1000)) {
                      console.log(`[OpenAI WebRTC] Processing similar transcript in listen mode:`, sourceText.substring(0, 30) + "...");
                    } else {
                      console.log(`[OpenAI WebRTC] Processing new transcript in listen mode:`, sourceText.substring(0, 30) + "...");
                    }
                    
                    // Update the timestamp for this message
                    processedMessagesMap[messageKey] = now;
                  }
  ...
  const sendToServer = (sourceText: string, translatedText: string, roomId: string) => {
    try {
      ...
      // For listen mode, use a timestamp-based deduplication approach
      if (isListenPage) {
        // Create a message fingerprint - use only the first 20 chars to avoid overly specific matching
        const messageKey = `${sourceText.substring(0, 20)}-${translatedText.substring(0, 20)}`;
        ...
        // Only block messages that were processed very recently (within 1 second)
        // This allows for better handling of similar messages that are legitimate retries
        if (lastProcessed && (now - lastProcessed < 1000)) {
          console.log(`[OpenAI WebRTC] Detected duplicate message in listen mode:`, sourceText.substring(0, 30) + "...");
          
          // Important: In Listen mode, we NEVER block messages to ensure smooth TTS
          // Just log it but still continue processing
          console.log(`[OpenAI WebRTC] Still allowing speech in listen mode for smooth playback`);
        } else {
