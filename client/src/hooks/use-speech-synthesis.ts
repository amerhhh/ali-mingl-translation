...
export function useSpeechSynthesis() {
  ...
  // Add a speech queue to manage sequential playback
  const speechQueue = useRef<Array<{
    text: string,
    lang: string,
    isManualPlayback: boolean
  }>>([]);
  ...
  const resetSpeechSynthesis = useCallback(() => {
    if ('speechSynthesis' in window) {
      ...
      // Clear the queue and reset processing flag when resetting
      speechQueue.current = [];
  ...
  const processQueue = useCallback(() => {
    // If no items in queue or already processing, just return
    if (speechQueue.current.length === 0 || isProcessingQueue.current) {
      return;
    }
    
    // Mark as processing to prevent multiple simultaneous processing
    isProcessingQueue.current = true;
    
    // Get the first item from the queue
    const item = speechQueue.current[0];
    
    // Process this item with the actual speak logic
    speakItem(item.text, item.lang, item.isManualPlayback)
      .then(() => {
        ...
        if (speechQueue.current.length > 0) {
          // Add a small delay between utterances for smoother experience
          setTimeout(() => {
            processQueue();
          }, 150);
        }
      ...
      .catch(() => {
        // Even on error, remove the item and continue
        speechQueue.current.shift();
        isProcessingQueue.current = false;
        
        // Try to process next item after a delay
        if (speechQueue.current.length > 0) {
          setTimeout(() => {
            processQueue();
          }, 300); // Longer delay after error
        }
      });
  }, []);
  
  // This is the actual function that handles speech synthesis - no longer directly called
  const speakItem = useCallback((text: string, lang: string = 'en-US', isManualPlayback: boolean = false): Promise<void> => {
      ...
      try {
        ...
        utterance.onerror = (event) => {
              ...
              if (!window.speechSynthesis.speaking) {
                try {
                  ...
                  newUtterance.onerror = () => {
                    ...
                    reject(new Error('Speech synthesis recovery failed'));
                  };
                  ...
                  (window as any).__speechRecoveryCount = 0;
                  setIsSpeaking(false);
                  reject(new Error('Speech synthesis recovery exception'));
                }
              } else {
                console.log('Speech already in progress, skipping recovery attempt');
                reject(new Error('Speech synthesis already in progress'));
              }
            }, recoveryDelay);
          ...
          toast({
            ...
            description: textHasArabicScript 
              ? "Arabic speech may require additional language packs on your device." 
        ...
        // Special handling for forced playback (used by OpenAI WebRTC integration)
        if ((window as any).__forcePlayNextUtterance) {
          console.log('[DEBUG] 🔊 Force playing this utterance (unblocked by force flag)');
          console.log(`[DEBUG] ✓ ALLOWING speech with text "${text.substring(0, 25)}..." (lang: ${langCode})`);
          
          // Reset the force flag immediately
  ...
  const speak = useCallback((text: string, lang: string = 'en-US', isManualPlayback: boolean = false) => {
    ...
    console.log(`Added text to speech queue. Queue length: ${speechQueue.current.length}`);
