...
export function useSpeechSynthesis() {
  ...
  const speak = useCallback((text: string, lang: string = 'en-US', isManualPlayback: boolean = false) => {
    ...
    try {
      ...
      // No special handling for Arabic - use the standard approach for all languages
      ...
      utterance.onerror = (event) => {
        ...
        // Special handling for Arabic text or other RTL languages - minimal approach
        if (textHasArabicScript) {
          // For Arabic text, if we get "interrupted" errors, just log it
          if (event.error === "interrupted" || event.error === "canceled") {
            console.log('Arabic speech was interrupted or canceled');
            // For simplicity, we won't attempt recovery - just silently continue
          }
        }
